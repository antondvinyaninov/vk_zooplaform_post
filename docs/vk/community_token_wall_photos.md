...
Практично: влить этот PR. В Mini App группы: Настройки сообщества → **«Разрешить загрузку фото на стену»**. VK сам выдаст токен вашего приложения (`VKWebAppGetAuthToken`, `photos`). Бэкенд проверяет `photos.getWallUploadServer` и только тогда сохраняет. Kate `/vk-connect` больше не используется.
