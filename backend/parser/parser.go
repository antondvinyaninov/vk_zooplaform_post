...
func isRelevantGroup(g vk.Group, currentCityID int, currentCityTitle string) bool {
	...
	animalPrefixes := []string{
		...
		"кинолог", "дрессиров", "зоонян", "догситер", "догситтер", "передержк", "сббж", "ветцентр", "лапк", "выгул",
	}
	animalExacts := []string{
	...
	animalPhrases := []string{
		"стрижка собак", "стрижка кошек", "стрижка животных",
		"ветеринарная аптека", "зооаптека", "зоо аптека",
	}

	if !hasPrefixInWords(animalPrefixes) && !hasExactInWords(animalExacts) && !containsPhrase(animalPhrases) {
		return false
	}
	...
	if hasPrefixInWords(salesPrefixes) || containsPhrase(salesPhrases) || hasExactInWords([]string{"доставка", "цена", "цены", "услуги"}) {
		allowedSalesPrefixes := []string{
			...
			"догситер", "догситтер", "зоонян", "передержк", "сббж", "стрижк", "выгул", "аптек", "постри",
		}
