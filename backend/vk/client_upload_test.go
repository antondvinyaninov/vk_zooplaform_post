...
func TestPhotoUploadResponseUploadPayload(t *testing.T) {
	empty := &PhotoUploadResponse{Photo: "[]", PhotosList: "  "}
	if empty.UploadPayload() != "" {
		t.Fatalf("empty payload %q", empty.UploadPayload())
	}
	fromList := &PhotoUploadResponse{PhotosList: `[{"photo":"x"}]`}
	if fromList.UploadPayload() != `[{"photo":"x"}]` {
		t.Fatalf("photos_list payload %q", fromList.UploadPayload())
	}
}

func TestPostPhotoToUploadURLRetriesFileField(t *testing.T) {
	var fields []string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.MultipartForm.File["photo"] != nil {
			field = "photo"
		} else if r.MultipartForm.File["file"] != nil {
			field = "file"
		}
		fields = append(fields, field)
		w.Header().Set("Content-Type", "application/json")
		if field == "photo" {
			io.WriteString(w, `{"server":1,"photo":"[]","hash":""}`)
			return
		}
		io.WriteString(w, `{"server":2,"photos_list":"[ok]","hash":"h"}`)
	}))
	t.Cleanup(srv.Close)
	...
	got, err := NewVKClient("").uploadPhotoFile(tmp, srv.URL)
	if err != nil {
		t.Fatalf("upload: %v", err)
	}
	if got.UploadPayload() != "[ok]" {
		t.Fatalf("payload %q", got.UploadPayload())
	}
	if strings.Join(fields, ",") != "photo,file" {
